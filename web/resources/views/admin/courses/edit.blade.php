...
@section('header-title', 'Edit Mata Kuliah')
@section('header-subtitle', 'Perbarui detail data mata kuliah perkuliahan')
...
@section('content')
    ...
    <div class="card" style="max-width: 700px;">
        <div class="card-header">
            <span class="card-title">Edit Mata Kuliah: {{ $course->name }}</span>
            ...
            <form action="{{ route('admin.courses.update', $course->id) }}" method="POST">
                ...
                <div class="grid-2" style="grid-template-columns: 1fr 1fr; gap: 18px;">
                    <div class="form-group">
                        ...
                        <input type="number" id="credits" name="credits" class="form-control" min="1" max="6" value="{{ old('credits', $course->credits) }}" required>
                    </div>

                    <div class="form-group">
                        <label for="lecturer_name" class="form-label">Nama Dosen Pengampu</label>
                        <input type="text" id="lecturer_name" name="lecturer_name" class="form-control" value="{{ old('lecturer_name', $course->lecturer_name) }}">
                    </div>
                </div>
                ...
                <div class="grid-2" style="grid-template-columns: 1fr 1fr; gap: 18px;">
                    <div class="form-group">
                        ...
                        <select id="schedule_day" name="schedule_day" class="form-control" required>
                            ...
                            <option value="0" {{ old('schedule_day', $course->schedule_day) == 0 ? 'selected' : '' }}>Senin</option>
                            <option value="1" {{ old('schedule_day', $course->schedule_day) == 1 ? 'selected' : '' }}>Selasa</option>
                            <option value="2" {{ old('schedule_day', $course->schedule_day) == 2 ? 'selected' : '' }}>Rabu</option>
                            <option value="3" {{ old('schedule_day', $course->schedule_day) == 3 ? 'selected' : '' }}>Kamis</option>
                            <option value="4" {{ old('schedule_day', $course->schedule_day) == 4 ? 'selected' : '' }}>Jumat</option>
                            <option value="5" {{ old('schedule_day', $course->schedule_day) == 5 ? 'selected' : '' }}>Sabtu</option>
                            <option value="6" {{ old('schedule_day', $course->schedule_day) == 6 ? 'selected' : '' }}>Minggu</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label for="room" class="form-label">Ruangan</label>
                        <input type="text" id="room" name="room" class="form-control" value="{{ old('room', $course->room) }}">
                    </div>
                </div>
                ...
                <div class="grid-2" style="grid-template-columns: 1fr 1fr; gap: 18px;">
                    <div class="form-group">
                        ...
                        <input type="time" id="schedule_start" name="schedule_start" class="form-control" value="{{ old('schedule_start', substr($course->schedule_start, 0, 5)) }}" required>
                    </div>

                    <div class="form-group">
                        <label for="schedule_end" class="form-label">Jam Selesai <span style="color: var(--danger);">*</span></label>
                        <input type="time" id="schedule_end" name="schedule_end" class="form-control" value="{{ old('schedule_end', substr($course->schedule_end, 0, 5)) }}" required>
                    </div>
                </div>
                ...
                <div class="grid-2" style="grid-template-columns: 1fr 1fr; gap: 18px;">
                    <div class="form-group">
                        ...
                        <input type="text" id="semester" name="semester" class="form-control" value="{{ old('semester', $course->semester) }}" required>
                    </div>
                </div>

                <div class="form-group" style="margin-top: 10px;">
                    <label style="display: inline-flex; align-items: center; gap: 8px; font-weight: 600; cursor: pointer;">
                        <input type="checkbox" name="is_active" value="1" style="width: 18px; height: 18px; accent-color: var(--primary);" {{ $course->is_active ? 'checked' : '' }}>
                        Mata Kuliah Aktif
                    </label>
                </div>
                ...
                <div style="border-top: 1px solid var(--border-color); margin-top: 24px; padding-top: 20px;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                        ...
                        <label id="loc-toggle-label" style="display: inline-flex; align-items: center; gap: 8px; font-weight: 600; cursor: pointer; padding: 8px 14px; border-radius: 8px; border: 1.5px solid var(--border-color); transition: all 0.2s;">
                            <input type="checkbox" id="location_required" name="location_required" value="1"
                                style="width: 18px; height: 18px; accent-color: var(--accent);"
                                {{ old('location_required', $course->location_required) ? 'checked' : '' }}>
                    ...
                    <div id="location-config" style="display: none;">
                        <div class="grid-2" style="grid-template-columns: 1fr 1fr; gap: 18px; margin-bottom: 14px;">
                            <div class="form-group">
                                ...
                                <input type="number" id="latitude" name="latitude" class="form-control" step="any"
                                    placeholder="-6.9824" value="{{ old('latitude', $course->latitude) }}">
                            </div>
                            <div class="form-group">
                                ...
                                <input type="number" id="longitude" name="longitude" class="form-control" step="any"
                                    placeholder="110.4121" value="{{ old('longitude', $course->longitude) }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group" style="max-width: 300px; margin-bottom: 14px;">
                            ...
                            <input type="number" id="location_radius" name="location_radius" class="form-control"
                                min="10" max="2000" placeholder="100"
                                value="{{ old('location_radius', $course->location_radius ?? 100) }}">
                        ...
                        <p style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 8px;">
                            ...
                            Klik pada peta atau geser marker untuk mengubah koordinat.
                        </p>
